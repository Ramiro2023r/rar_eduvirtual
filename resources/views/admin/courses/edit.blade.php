        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.courses.update', $course) }}">
                ...
                <div class="form-group">
                    ...
                    <select name="professor_id" id="professor_id" class="form-control">
                        ...
                        @foreach($profesores as $prof)
                            <option value="{{ $prof->id }}" {{ old('professor_id', $course->professor_id) == $prof->id ? 'selected' : '' }}>
                                {{ $prof->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="credits" id="credits" class="form-control" value="{{ old('credits', $course->credits) }}" min="1" max="10" required>
                ...
                <div class="form-group">
                    ...
                    <select name="status" id="status" class="form-control" required>
                        <option value="activo" {{ $course->status === 'activo' ? 'selected' : '' }}>Activo</option>
                        <option value="inactivo" {{ $course->status === 'inactivo' ? 'selected' : '' }}>Inactivo</option>
                    </select>
